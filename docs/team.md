---
title: Team
---

# Who we are

URBADAPT is developed at the **[CMCC Foundation](https://www.cmcc.it)** — the Euro-Mediterranean
Center on Climate Change — within the
**[ECIP Division](https://www.cmcc.it/what-we-do/institutes/european-institute-on-economics-and-the-environment-eiee/economic-analysis-of-climate-impacts-and-policy-division)**
(Economic analysis of Climate Impacts and Policy), part of the
[European Institute on Economics and the Environment (EIEE)](https://www.eiee.org).

ECIP's work centres on translating the physical impacts of climate change into
economic terms and on evaluating the policies available to adapt to them.
URBADAPT sits squarely in that agenda: it takes high-resolution physical climate
hazard down to the level at which a city actually makes decisions, and attaches
costs, benefits, and distributional consequences to the adaptation options on the
table.

---

## People

<div class="grid cards" markdown>

-   :material-account-star:{ .lg .middle } __Giacomo Falchetta__

    ---

    **Principal Investigator**

    CMCC · EIEE (ECIP Division)

    Leads the URBADAPT programme: research direction, framework design, and the
    economic appraisal of adaptation investment.

    [:octicons-mail-24: giacomo.falchetta@cmcc.it](mailto:giacomo.falchetta@cmcc.it)

-   :material-code-braces:{ .lg .middle } __Armande Aboudrar-Méda__

    ---

    **Lead developer**

    CMCC · EIEE (ECIP Division)

    Builds and maintains the URBADAPT-HEAT modelling pipeline — the geospatial
    processing chain, the `cityheat` package, and the city-agnostic notebook
    workflow.

</div>

---

## Get in touch

For questions about the framework, collaboration proposals, or requests to add a
city, contact **Giacomo Falchetta** at
[giacomo.falchetta@cmcc.it](mailto:giacomo.falchetta@cmcc.it).

Bug reports and feature requests are best raised as
[issues on GitHub](https://github.com/URBADAPT/URBADAPT-HEAT/issues), where they
stay visible to everyone using the framework.

---

## Institutions

| | |
|---|---|
| [CMCC Foundation](https://www.cmcc.it) | Euro-Mediterranean Center on Climate Change |
| [EIEE](https://www.eiee.org) | European Institute on Economics and the Environment, CMCC |
| [ECIP Division](https://www.cmcc.it/what-we-do/institutes/european-institute-on-economics-and-the-environment-eiee/economic-analysis-of-climate-impacts-and-policy-division) | Economic analysis of Climate Impacts and Policy |
